...
| US-08    | **Dashboard de operaciones (Filtrado)**      | Como operador logístico, quiero listar los envíos filtrando por estado (ej. solo "En Tránsito") o fecha, para priorizar mi atención en los activos activos.                                       | **Escenario 1: Filtro por estado.**<br>Given existen 50 envíos (20 entregados, 30 en tránsito)<br>When filtro por "IN_TRANSIT"<br>Then obtengo una lista paginada de 30 envíos.<br><br>**Escenario 2: Búsqueda vacía.**<br>When no envío filtros<br>Then obtengo los últimos 20 envíos creados (paginación por defecto).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| US-09    | **Cancelación de envíos**                    | Como operador lógistico, quiero cancelar un envío que fue registrado por error, para mantener limpio el inventario.                                                                               | **Escenario 1: Cancelación válida.**<br>Given un envío en estado "CREATED"<br>When solicito cancelar<br>Then el envío se elimina o pasa a estado "CANCELLED".<br><br>**Escenario 2: Regla de invariante.**<br>Given un envío "IN_TRANSIT"<br>When intento cancelar<br>Then el sistema rechaza la operación (ya salió del almacén).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| US-10    | **Actualización de envío (Edición)**         | Como operador lógistico, quiero corregir los datos de un envío (descripción, contacto, límites) antes de que salga, para subsanar errores manuales.                                               | **Escenario 1: Edición exitosa (Happy Path).**<br>Given un envío en estado "CREATED"<br>When modifico la temperatura máxima de 5.0 a 6.0<br>Then el sistema guarda los cambios.<br><br>**Escenario 2: Bloqueo por invariante.**<br>Given un envío que ya salió ("IN_TRANSIT")<br>When intento modificar los rangos de temperatura<br>Then el sistema rechaza la edición (las reglas no se cambian a mitad de viaje).                                                                                                                                                                                                                                                                                                                                                                                                                                           |

### 3. Arquitectura de Software
Se ha diseñado una arquitectura basada en Clean Architecture y Domain-Driven Design (DDD). A continuación, se detallan los diferentes niveles de abstracción según el modelo C4.

#### 3.1. Diagrama de contexto (C4 - Nivel 1)
Este diagrama ilustra el panorama general del sistema PharmaCold Track, identificando las interacciones con los actores externos (Operador y sensores) y sistemas terceros (Servicio de correo).

![PharmaCold_Track_Context_Diagram.png](assets/PharmaCold_Track_Context_Diagram.png)
